...
float ry = para[6];
float my1 = para[7];
//double m0 = para[8];double m2 = para[8];double m3 = para[9];
float d;
double deltay;
...
while (q<136) {
  ...
  ypix = xprime.Y()*magy;
   ...
   //h->Fill(ypix,yc2[q]);
   deltay = ypix - yc2[q];  cout<<xc2[q]<<" "<<yc2[q]<<" "<<deltay<<endl;
   //r = sqrt((TMath::Power((xc2[q]-xpix),2))+(TMath::Power((yc2[q]-ypix),2))); 
   h->SetBinContent((xc2[q]+2155)/(4310/bin),(yc2[q]+1434)/(2868/bin),deltay);   
   q++;
}
...
h->SetTitle("8 parameters delta y");
