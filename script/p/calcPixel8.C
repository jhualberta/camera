...
double calcPixel(double *x, double *p){
  int iq=floor(x[0]);
  int q=iq/2;
  double  alphax = p[0];                           //magnification constant for x
  ...
  double alphay = p[7];                            //magnification constant for y
...
void myfunc()
{
   ...
   c->Divide(1,2);
   c->cd(1);
   TF1 *f1 = new TF1("calcP",calcPixel,0,272,8);
   f1->SetNpx(272);
   f1->SetParameters(1.95e3,7289.7,193.5,4246.6,15.5*TMath::Pi()/180, 6.75*TMath::Pi()/180, 1.5*TMath::Pi()/180, 1.95e3);
