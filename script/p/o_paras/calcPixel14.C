...
double calcPixel(double *x, double *p){
  ...
  double mx0 = p[7]; double mx2=p[8]; double mx3 = p[9];      //adjustment to magnification for x
  double my1 = p[10]; 
  double my0 = p[11]; double my2=p[12]; double my3 = p[13];      //adjustment to magnification for y
...
void myfunc()
{
   ...
   c->Divide(1,2);
   c->cd(1);
   TF1 *f1 = new TF1("calcP",calcPixel,0,272,14);
   f1->SetNpx(272);
   double p[14]={1.95e3,7289.7,193.5,4246.6,15.5*TMath::Pi()/180, 6.75*TMath::Pi()/180, 1.5*TMath::Pi()/180,0,0,0,1.95e3,0,0,0};
   f1->SetParameters(p);
