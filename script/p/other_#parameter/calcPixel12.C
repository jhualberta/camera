...
double calcPixel(double *x, double *p){
  ...
  double ry = p[5];                
  double mx1 = p[6];//adjustment to magnification for x
//  double mx2 = p[7]; 
 // double mx3 = p[8];                  
  //double my1 = p[7]; //adjustment to magnification for y
  //double my2 = p[10]; 
  //double my3 = p[11];
...
//cout<<"out"<<" "<<out<<endl;
 ...
 // cout<<"aob2"<<" "<<aob2<<endl;
  ...
  diy = theta*mx1; //+ my2*theta*theta + my3*theta*theta*theta;
  xpix = dix*(obxy*i2d);
  ypix = diy*(obxy*j2d);
...
void myfunc()
{
   ...
   c1->Divide(1,2);
   c1->cd(1);
   TF1 *f1 = new TF1("calcP",calcPixel,0,272,7);
   f1->SetNpx(272);

   double p[7]={7289.7,193.5,4246.6,0.272524, 0.127128, 0.027407,1900};
   f1->SetParameters(p);
