...
float xc[152], yc[152], zc[152];
float number[152], xm[152], ym[152];
...
void myfunc()
{
   ...
   c1->Divide(1,2);
   c1->cd(1);
   TF1 *f1 = new TF1("calcP",calcPixel,0,304,12);
   f1->SetNpx(304);
   double p[12]={7289.7, 193.5, 4246.6, 0.272524, 0.127128, 0.027407, 1900, 0, 0, 0, 0, 0};
   f1->SetParameters(p);
   f1->FixParameter(0,7289.7);
   f1->FixParameter(1,193.5);
   f1->FixParameter(2,4246.6);    


   //f1->FixParameter(3,0.273702);
   //f1->FixParameter(4,0.105137);
   //f1->FixParameter(5,0.0241625);
   //f1->FixParameter(6,2025.26);
   //f1->FixParameter(7,1980.13);
   //f1->FixParameter(8,-15.9674);
   //f1->FixParameter(9,-142.474);++
   //f1->FixParameter(10,12.9136);
   //f1->FixParameter(11,-61.531);
...
//   f1->SetParNames("constant","coefficient",);
   f1->Draw();
   TH1F *h1=new TH1F("h1","test",304,0,304);
   for(int i=0;i<152;i++){
      h1->SetBinContent(2*i+1,xm[i]); h1->SetBinContent(2*i+2,ym[i]);
      //cout<<"xm"<<i<<" "<<xm[i]<<endl;
      //cout<<"ym"<<i<<" "<<ym[i]<<endl;
   }
   for(int i=1;i<=304;i++)h1->SetBinError(i,30);
   h1->Fit("calcP");
  ...
  // h1->Fit("calcP");
   ...
   f1->SetParameter(7,f1->GetParameter(6));
   ...
   h1->Fit("calcP");
   f1->ReleaseParameter(11);
   h1->Fit("calcP");



   TH1F *h2=new TH1F("h2","residual",304,0,304);
   for(int i=0;i<304;i++)h2->SetBinContent(i+1,h1->GetBinContent(i+1)-f1->Eval(i));
   c1->cd(2); h2->Draw();
