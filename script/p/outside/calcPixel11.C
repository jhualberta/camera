...
float xc[90], yc[90], zc[90];
float number[90], xm[90], ym[90];

double calcPixel(double *x, double *p){
  int iq=floor(x[0]);
  int q=iq/2;
  double theta_cam; double phi_cam; double r_cam = 8438;
  theta_cam = p[0];
  phi_cam = p[1];
...
void myfunc()
{
   ...
   c1->Divide(1,2);
   c1->cd(1);
   TF1 *f1 = new TF1("calcP",calcPixel,0,180,11);
   f1->SetNpx(180);
   double p[11]={1.04, 0.07, 0.272524, 0.127128, 0.027407, 1900, 1900, 0, 0, 0, 0};
   f1->SetParameters(p);
   f1->FixParameter(2,0);
   f1->FixParameter(3,0);
   f1->FixParameter(4,0);    
   ...
   f1->Draw();
   TH1F *h1=new TH1F("h1","test",180,0,180);
   for(int i=0;i<90;i++){
      h1->SetBinContent(2*i+1,xm[i]); h1->SetBinContent(2*i+2,ym[i]);
      //cout<<"xm"<<i<<" "<<xm[i]<<endl;
      //cout<<"ym"<<i<<" "<<ym[i]<<endl;
   }
   for(int i=1;i<=180;i++)h1->SetBinError(i,30);
   h1->Fit("calcP");
   ...
   TH1F *h2=new TH1F("h2","residual",180,0,180);
   for(int i=0;i<180;i++)h2->SetBinContent(i+1,h1->GetBinContent(i+1)-f1->Eval(i));
   c1->cd(2); h2->Draw();
