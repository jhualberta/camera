...
float xc[53], yc[53], zc[53];
float number[53], xm[53], ym[53];

double calcPixel(double *x, double *p){
  int iq=floor(x[0]);
  int q=iq/2;
  double theta_cam; double phi_cam; double r_cam = 8401.64;
  theta_cam = p[0];
  phi_cam = p[1];
...
void myfunc()
{
   ...
   c1->Divide(1,2);
   c1->cd(1);
   TF1 *f1 = new TF1("calcP",calcPixel,0,106,11);
   f1->SetNpx(106);
   double p[11]={1.88, 1.05, -2.09, 0, 0, 1900, 1900, 0, 0, 0, 0};
   f1->SetParameters(p);
   //f1->FixParameter(2,0);
   f1->FixParameter(3,0);
   f1->FixParameter(4,0);    
   ...
   f1->Draw();
   TH1F *h1=new TH1F("h1","test",106,0,106);
   for(int i=0;i<53;i++){
      h1->SetBinContent(2*i+1,xm[i]); h1->SetBinContent(2*i+2,ym[i]);
      //cout<<"xm"<<i<<" "<<xm[i]<<endl;
      //cout<<"ym"<<i<<" "<<ym[i]<<endl;
   }
   for(int i=1;i<=106;i++)h1->SetBinError(i,30);
   h1->Fit("calcP");
   ...
   TH1F *h2=new TH1F("h2","residual",106,0,106);
   for(int i=0;i<106;i++)h2->SetBinContent(i+1,h1->GetBinContent(i+1)-f1->Eval(i));
   c1->cd(2); h2->Draw();
