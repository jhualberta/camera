...
float xc[42], yc[42], zc[42];
float number[42], xm[42], ym[42];
...
void myfunc()
{
   ...
   c1->Divide(1,2);
   c1->cd(1);
   TF1 *f1 = new TF1("calcP",calcPixel,0,84,11);
   f1->SetNpx(84);
   double p[11]={1.07, 1.89, -2.09, 0, 0, 1950, 1950, 0, 0, 0, 0};
   f1->SetParameters(p);


   f1->Draw();
   TH1F *h1=new TH1F("h1","test",84,0,84);
   for(int i=0;i<42;i++){
      h1->SetBinContent(2*i+1,xm[i]); h1->SetBinContent(2*i+2,ym[i]);
      //cout<<"xm"<<i<<" "<<xm[i]<<endl;
      //cout<<"ym"<<i<<" "<<ym[i]<<endl;
   }
   for(int i=1;i<=84;i++)h1->SetBinError(i,30);
   h1->Fit("calcP");


   TH1F *h2=new TH1F("h2","residual",84,0,84);
   for(int i=0;i<84;i++)h2->SetBinContent(i+1,h1->GetBinContent(i+1)-f1->Eval(i));
   c1->cd(2); h2->Draw();
