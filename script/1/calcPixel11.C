...
float xc[74], yc[74], zc[74];
float number[74], xm[74], ym[74];

double calcPixel(double *x, double *p){
  int iq=floor(x[0]);
  int q=iq/2;
  double theta_cam; double phi_cam; double r_cam = 8558;
  theta_cam = p[0];
  phi_cam = p[1];
...
void myfunc()
{
   ...
   c1->Divide(1,2);
   c1->cd(1);
   TF1 *f1 = new TF1("calcP",calcPixel,0,148,11);
   f1->SetNpx(148);
   double p[11]={-2.137, 3.13, 0, 0, 0, 1900, 1900, 0, 0, 0, 0};
   f1->SetParameters(p);
   f1->FixParameter(2,0);
   f1->FixParameter(3,0);
   f1->FixParameter(4,0);    
   ...
   f1->Draw();
   TH1F *h1=new TH1F("h1","test",148,0,148);
   for(int i=0;i<74;i++){
      h1->SetBinContent(2*i+1,xm[i]); h1->SetBinContent(2*i+2,ym[i]);
      //cout<<"xm"<<i<<" "<<xm[i]<<endl;
      //cout<<"ym"<<i<<" "<<ym[i]<<endl;
   }
   for(int i=1;i<=148;i++)h1->SetBinError(i,30);
   h1->Fit("calcP");
   ...
   TH1F *h2=new TH1F("h2","residual",148,0,148);
   for(int i=0;i<148;i++)h2->SetBinContent(i+1,h1->GetBinContent(i+1)-f1->Eval(i));
   c1->cd(2); h2->Draw();
