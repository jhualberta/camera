...
float xc[47], yc[47], zc[47];
float number[47], xm[47], ym[47];
...
void myfunc()
{
   ...
   c1->Divide(1,2);
   c1->cd(1);
   TF1 *f1 = new TF1("calcP",calcPixel,0,94,11);
   f1->SetNpx(94);
   ...
   f1->Draw();
   TH1F *h1=new TH1F("h1","test",94,0,94);
   for(int i=0;i<47;i++){
      h1->SetBinContent(2*i+1,xm[i]); h1->SetBinContent(2*i+2,ym[i]);
      //cout<<"xm"<<i<<" "<<xm[i]<<endl;
      //cout<<"ym"<<i<<" "<<ym[i]<<endl;
   }
   for(int i=1;i<=94;i++)h1->SetBinError(i,30);
   h1->Fit("calcP");
   ...
   TH1F *h2=new TH1F("h2","residual",94,0,94);
   for(int i=0;i<94;i++)h2->SetBinContent(i+1,h1->GetBinContent(i+1)-f1->Eval(i));
   c1->cd(2); h2->Draw();
