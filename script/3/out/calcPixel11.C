...
void myfunc()
{
   ...
   c1->Divide(1,2);
   c1->cd(1);
   TF1 *f1 = new TF1("calcP",calcPixel,0,96,11);
   f1->SetNpx(96);
   double p[11]={-0.8, 1.06, 2.2, 0.044, -0.175, 2000, 2000, 0, 0, 0, 0};
   f1->SetParameters(p);
 

   f1->Draw();
   TH1F *h1=new TH1F("h1","test",96,0,96);
   for(int i=0;i<48;i++){
      h1->SetBinContent(2*i+1,xm[i]); h1->SetBinContent(2*i+2,ym[i]);
      //cout<<"xm"<<i<<" "<<xm[i]<<endl;
      //cout<<"ym"<<i<<" "<<ym[i]<<endl;
   }
   for(int i=1;i<=96;i++)h1->SetBinError(i,30);
   h1->Fit("calcP");

  

   TH1F *h2=new TH1F("h2","residual",96,0,96);
   for(int i=0;i<96;i++)h2->SetBinContent(i+1,h1->GetBinContent(i+1)-f1->Eval(i));
   c1->cd(2); h2->Draw();
