...
double av_pos[3];
ifstream myReadFilea;
myReadFilea.open("../av_pos.txt");
in=0;
if (myReadFilea.is_open()) {
while (!myReadFilea.eof()) {

   myReadFilea >> av_pos[in];
   in++;
}
}
myReadFilea.close();
...
for(int h=0; h<3; h++){
TVector3 av(av_pos[0],av_pos[1],av_pos[2]);
rad = rad+100;
